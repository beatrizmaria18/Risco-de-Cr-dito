...
def prepare_input(form_data):
    ...
    # Feature engineering (igual ao que você fez no treinamento)
    ...
    for col in model['categorical_features']:
        if col in input_df.columns:
            # Obter todas as categorias que o encoder espera
            categories = model['onehot_encoder'].categories_[model['categorical_features'].index(col)]
            
            # Para cada categoria possível, criar uma coluna
            for category in categories:
                new_col_name = f"{col}_{category}"
                input_df[new_col_name] = (input_df[col] == category).astype(int)
    ...
    for col in missing_cols:
        input_df[col] = 0  # Preencher com zero colunas faltantes
    
    # Manter apenas as colunas na ordem esperada pelo modelo
    input_df = input_df[model['feature_names']]
    
    # Aplicar scaling nas variáveis numéricas
    numeric_cols = model['numerical_features']
    input_df[numeric_cols] = model['scaler'].transform(input_df[numeric_cols])
    
    return input_df

# Exemplo de uso no formulário Streamlit
with st.form("risk_form"):
    # Seus campos de entrada originais
    emprego = st.selectbox("Emprego", options=['Autônomo', 'Comissionado', 'Geral', 'Gerente', 'Outros', 'Professional'])
    finalidade = st.selectbox("Finalidade", options=['Reforma', 'Diversas'])
    # ... outros campos conforme seu formulário original
    ...
    if st.form_submit_button("Calcular Risco"):
        ...
        form_data = {
            ...
            # ... todos os outros campos do seu formulário
        }
        ...
        try:
            ...
            probability = model['model'].predict_proba(input_data)[0][1]
            
            st.success(f"Probabilidade de mau pagador: {probability:.1%}")
            
        except Exception as e:
            st.error(f"Erro ao processar: {str(e)}")
            st.write("Dados enviados:", form_data)
